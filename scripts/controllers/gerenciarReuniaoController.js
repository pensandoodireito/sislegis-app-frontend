angular
		...
		.controller(
				...
				function($scope, $rootScope, $http, $filter, $routeParams, $location, $modal, $log, $timeout, toaster,
						...
						EquipeResource, ReuniaoProposicaoResource, TagResource, EncaminhamentoProposicaoResource,
						ComentarioService, UsuarioResource, ElaboracaoNormativaResource, BACKEND) {
					...
					$scope.loadTags = function(query) {
						return TagResource.buscarPorSufixo({
							sufixo : query
						}).$promise;
					};

					$scope.loadProposicoes = function(query) {
						return ProposicaoResource.buscarPorSufixo({
							sufixo : query
						}).$promise;
					};

					$scope.loadElaboracoesNormativas = function(query) {
						return ElaboracaoNormativaResource.buscarPorSufixo({
							sufixo : query
						}).$promise;
					};
					...
					$scope.save = function(item) {
						...
						var successCallback = function() {
							...
							$scope.tagsProposicao = TagResource.listarTodos();
							ReuniaoResource.buscarReuniaoPorData({
								data : $scope.dataFormatada()
							}, function(response) {
					...
					$scope.alterarPosicionamento = function(item) {
						...
						var idPosicionamento = $scope.selectedProposicao.posicionamento ? $scope.selectedProposicao.posicionamento.id
								: null;
						ProposicaoResource.alterarPosicionamento({
							id : $scope.selectedProposicao.id,
							idPosicionamento : idPosicionamento
						}, successCallback, errorCallback);
					...
					$scope.dataFormatada = function() {
						var formattedDate = $filter('date')(new Date($scope.reuniao.data), 'MM/dd/yyyy');
										...
										if (!angular.isUndefined($scope.reuniao.data)) {
											...
											var successCallback = function() {
												if ($scope.listaReuniaoProposicoes.length == 0) {
													toaster
															.pop('info',
																	'Não existem proposições para esta data. Você pode adicionar novas proposições.');
												}
											};
											var errorCallback = function() {
											...
											$scope.listaReuniaoProposicoes = ReuniaoResource.buscarReuniaoPorData({
												data : $scope.dataFormatada()
											}, successCallback, errorCallback);
										}
					...
					$scope.abrirModalComentarios = function(item) {
						$scope.selectedProposicao = item;
						if ($scope.selectedProposicao.listaComentario == null
								|| $scope.selectedProposicao.listaComentario.length != $scope.selectedProposicao.totalComentarios) {
							ComentarioResource.findByProposicao({
								ProposicaoId : $scope.selectedProposicao.id
							}, $scope.populaModalComentario);
						} else {
					...
					$scope.abrirModalRemoverProposicao = function(item) {
						$scope.selectedProposicao = item;
						console.log("a", $scope.reuniao, $scope.dataFormatada())
						var modalInstance = $modal.open({
					...
					$scope.abrirModalEncaminhamentos = function(item) {
						...
						if ($scope.selectedProposicao.listaEncaminhamentoProposicao == null
								|| $scope.selectedProposicao.listaEncaminhamentoProposicao.length != $scope.selectedProposicao.totalEncaminhamentos) {
							EncaminhamentoProposicaoResource.findByProposicao({
								ProposicaoId : $scope.selectedProposicao.id
							}, $scope.populaModalEncaminhamentos);
						} else {
...
angular.module('sislegisapp').controller(
		'ModalRemoverProposicaoController',
		function($scope, $http, $filter, $routeParams, $location, toaster, $modalInstance, proposicao, reuniao, data,
				ProposicaoResource, ReuniaoProposicaoResource, ComentarioResource, ComentarioService) {
			...
			$scope.removerProposicao = function() {
				...
				ReuniaoProposicaoResource.remove({
					ReuniaoId : $scope.data,
